...
- Clang + `libclang` (or an LLVM installation that provides `libclang`) for Rust `bindgen`
- `ar`
- `automake`
- `autoconf`
- `libtool`
- `make`
- `pkg-config` (optional; only needed on some platforms/build environments)

`ralfpack` uses `bindgen` in `build.rs`, so `clang`/`libclang` must be installed and discoverable even if you compile the C code with `gcc`.
